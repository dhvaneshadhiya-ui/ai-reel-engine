...
_kit = Reel(slug=SLUG, avatar=AVATAR, clips=C,
           credit=CRED if 'CRED' in dir() else '', focus_x=FOCUS_FULL)
...
def flo(png, aid, aspect, credit, sfx=None):
    """A still. Was `floatcard`, which crashes the renderer (G35) because
    floatcard renders an <OffthreadVideo> and ffmpeg decodes a PNG as one
    frame. reelkit.doc() emits a `receipt` and MEASURES the file, so the
    highlights land on the right pixels. `aspect` is ignored.
    """
    return _kit.doc(f"{png}.png", aid, credit=credit, sfx=sfx)
