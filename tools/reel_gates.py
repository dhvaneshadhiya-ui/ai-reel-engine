...
BLOCKING_RULES: dict[str, str] = {
    ...
    "G56": "R1 display type that never lands leaves a muted beat with no words",
...
def check_beats(beats: dict, vo_end: float | None = None,
    ...
    # G56 — TYPECARD AND KINETIC TYPE MUST LAND TOO (2026-09-01).
    #
    ...
    for i, sc in enumerate(scenes):
        ...
        if isinstance(ats, list):
            for c, a in enumerate(ats[:len(claims)]):
                if a > dur - 0.01:
                    errors.append(
                        f"G56 scene {i:02d} {what}: claim {c} "
        ...
        if last + ROW_DWELL > dur + 0.01 and last <= dur - 0.01:
            errors.append(
                f"G56a scene {i:02d} {what}: the last line lands at "
