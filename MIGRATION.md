...
| `bin/` (ffmpeg, ffprobe) | ~150 MB | static builds, too large for git | `brew install ffmpeg`. Copying `bin/` across is **not enough on its own** — see below |
...
> **`bin/` is a transport, not an install.** Every ffmpeg call in this repo is a
> bare `ffmpeg` / `ffprobe` through `subprocess.run` — about twenty of them,
> across `reel_gates`, `pace_reel`, `lint_frames`, `capture.mjs` and the rest —
> so the binary is found on **PATH**, always. Nothing reads `bin/`, and nothing
> puts `bin/` on PATH. Copying `bin/` across therefore gives you 150 MB that no
> tool can reach. If you do copy it, link it:
>
> ```bash
> ln -s "$PWD/bin/ffmpeg"  /usr/local/bin/ffmpeg
> ln -s "$PWD/bin/ffprobe" /usr/local/bin/ffprobe
> ```
>
> On Apple Silicon don't bother — macOS deletes the unsigned x86_64 build the
> first time it runs (§2.1, §5). Use `brew install ffmpeg`.
